<x-app-layout>
    <style>
        ...
        .btn-pay { width: 100%; background: var(--accent); color: #000; padding: 1.2rem; border-radius: 100px; border: none; font-weight: 900; font-size: 1.1rem; cursor: pointer; margin-top: 2rem; box-shadow: var(--glow); font-style: italic; text-transform: uppercase; transition: 0.2s; }
        .btn-pay:hover:not(:disabled) { transform: translateY(-3px); box-shadow: 0 10px 25px rgba(29, 185, 84, 0.4); }
        ...
        .btn-voucher:hover { background: var(--accent); color: #000; box-shadow: var(--glow); }
                ...
                <form id="checkoutForm">
                    ...
                    <div style="background: var(--bg-input); padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 1px solid var(--border);">
                        ...
                        <div class="input-group" style="margin-bottom: 0;">
                            <label class="input-label">Email (Untuk pengiriman e-Ticket)</label>
                            <input type="email" name="emails[]" class="input-box ticket-email" value="" placeholder="email@contoh.com" required>
                        </div>
                    </div>
                    @endfor

                    <h2 class="c-title" style="margin-top: 3rem;">Kontak Utama</h2>
                    <div class="input-group">
                        ...
                        <small style="color: var(--text-sub); font-size: 0.75rem; margin-top:5px; display:block;">*Pastikan nomor aktif untuk keperluan informasi event.</small>
                    </div>
            ...
            <div class="c-card" style="position: sticky; top: 100px;">
                ...
                <div style="margin-top: 2.5rem; margin-bottom: 1.5rem; border-top: 1px solid var(--border); padding-top: 1.5rem;">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <input type="text" id="voucher_code" class="input-box" placeholder="Masukkan Kode" style="margin-bottom:0; text-transform:uppercase;">
                        <button type="button" class="btn-voucher" onclick="applyVoucher()" id="btn_apply_voucher">TERAPKAN</button>
                    </div>
                </div>

                <button type="button" class="btn-pay" id="btn_pay_main" onclick="processCheckout()">BAYAR SEKARANG</button>
            </div>
        </div>
    </div>

    <script>
        // === LOGIKA VOUCHER REAL-TIME ===
        function applyVoucher() {
            let code = document.getElementById('voucher_code').value.toUpperCase();
            if(!code) return alert('Masukkan kode voucher dulu!');

            const btn = document.getElementById('btn_apply_voucher');
            btn.innerText = 'Cek...'; btn.disabled = true;

            fetch('{{ route("checkout.voucher.check") }}', {
                method: 'POST',
            ...
            .then(res => res.json())
            .then(data => {
                btn.innerText = 'TERAPKAN'; btn.disabled = false;
                if(data.success) {
                    document.getElementById('discount_row').style.display = 'flex';
                    document.getElementById('discount_amount').innerText = '- Rp ' + data.discount_formatted;
                    document.getElementById('final_total').innerText = 'Rp ' + data.final_formatted;
        ...
        // === LOGIKA PEMBAYARAN LANGSUNG (TANPA OTP) ===
        function processCheckout() {
            ...
            if(!wa || names.includes('') || emails.includes('')) {
                return alert('Lengkapi semua data tiket dan WA dulu ya!');
            }

            const btn = document.getElementById('btn_pay_main');
            btn.innerText = "MEMPROSES PEMBAYARAN... ⏳"; btn.disabled = true;
